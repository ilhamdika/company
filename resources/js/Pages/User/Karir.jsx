...
export default function Karir({rekruitmens, administrasis, mental_ideologis, kemampuan_fisiks, tugas_pokoks, fisik_mentals, akademiks, keterampilans, bentuk_kerjasamas}) {
    ...
    return (
        ...
        <div className="items-left justify py-2 laptop:px-20">
            ...
            <p className="mt-1 dark:text-white" data-aos="fade-right" key={administrasi.id}><span className="font-semibold">{i+1}. </span> {administrasi.title}</p>
            ))}
            ...
            <p className="mt-1 dark:text-white" data-aos="fade-right" key={mental.id}><span className="font-semibold" >{i+1}. </span> {mental.title}</p>
            ))}
            ...
            <p className="mt-1 dark:text-white" data-aos="fade-right" key={fisik.id}><span className="font-bold">{i+1} .</span> {fisik.title}</p>
            ))}
        </div>
        <div className="grid grid-cols-2 laptop:flex-row dekstop:flex-row mobile:grid-cols-1 mt-3 px-10 mobile:px-1">
                ...
                <div className="grid grid-cols-2 laptop:flex-row dekstop:flex-row mobile:grid-cols-1 mt-3 px-10 mobile:px-1">
                        <div className='mt-10 mobile:h-100 mobile:w-full'>
                            <div className=" dark:text-white rounded-md">
                                ...
                                <p className="mt-2" key={tugas.id} data-aos="fade-up-right"><span className="font-semibold">{i+1}. </span>{tugas.title}</p>
                                ))}
                                <div className="pt-4" data-aos="fade-up" data-aos-anchor-placement="top-bottom">
                                    <Link href={route('user.hubungi-kami')}>
                ...
                <div className="items-left justify laptop:px-10">
                    ...
                    {bentuk_kerjasamas.map((tugas, i = 1) => (
                        <p className="mt-2 dark:text-white" data-aos="fade-up-right" key={tugas.id}><span className="font-semibold">{i+1}. </span>{tugas.title}</p>
                    ))}
                </div>
        </LayoutPage>
    )
